...
#
# Variables:
#   %ecx - will hold the count of charecters proccessed
...
conversion_loop:
    ...
    # an index on the ACII table starting from the
    # character '0'. The acii code for '0' plus zero
    # is still the acii code for '0'. The ascii code
    ...
    # stack. This wat, when we are done, we can just
    # pop off the characters one-by-one and they will
    # be in the rigth order. Note that we are pushing
...
copy_reversing_loop:
    ...
    # small part (%al) into the charecter string.
    popl  %eax
    movb  %al, (%edx)
    ...
    cmpl  $0, %ecx
    # If so, jump to the endof the function
